...
void HospitalControlApp::readFileDirect() {
    std::string str_line;
    std::ifstream MyReadFile("weightEdges.txt");
    while (getline(MyReadFile, str_line)) {
        directIntersection::readLineDirect(str_line, &directIn);
    }
    MyReadFile.close();
}

void HospitalControlApp::readFileIVertex(std::vector<std::string> *iVertex) {
    std::string str_line;
    std::ifstream MyReadFile("i-vertex.txt");
    while (getline(MyReadFile, str_line)) {
        iVertex->push_back(str_line);
    }
    MyReadFile.close();
}
...
//if period is 0.2 => count = 801
    ...
    EV << "As 10 + 1(flow 10) + 1(flow11) AGVs => T: 4793, W: 2416, %: 50%"
            << endl;
    EV << "As 10 + 1(flow 11) + 1(flow 2) AGVs => T: 5175, W: 2782, %: 54%"
            << endl;
    EV
            << "As 10 + 1(flow10) + 1(flow 11) + 1(flow 2) AGVs => T: 5455, W: 2871, %: 53%"
            << endl;
    EV
            << "As 10 + 2(flow10) + 1(flow 11) + 1(flow 2) AGVs => T: 6846, W: 3616, %: 53%"
            << endl;
    EV
            << "As 10 + 2(flow10) + 2(flow 11) + 1(flow 2) AGVs => T: 5863, W: 2850, %: 49%"
            << endl;
    EV
            << "As 10 + 2(flow10) + 2(flow 11) + 1(flow 2) + 1(flow0) AGVs => T: 6734, W: 3517, %: 52%"
            << endl;
    EV
            << "As 10 + 2(flow10) + 2(flow 11) + 3(flow0-2) AGVs => T: 7649, W: 4202, %: 55%"
            << endl;
    EV
            << "As 10 + 2(flow10) + 2(flow 11) + 4(flow0-3) AGVs => T: 7261, W: 3621, %: 50%"
            << endl;
    EV
            << "As 10 + 2(flow10) + 2(flow 11) + 5(flow0-4) AGVs => T: 6488, W: 2644, %: 41%"
            << endl;
    EV
            << "As 10 + 2(flow10) + 2(flow 11) + 6(flow0-5) AGVs => T: 8027, W: 3973, %: 49.5%"
            << endl;
    EV
            << "As 10 + 2(flow10) + 2(flow 11) + 7(flow0-6) AGVs => T: 9578, W: 5289, %: 55%"
            << endl;
    EV
            << "As 10 + 2(flow10) + 2(flow 11) + 8(flow0-7) AGVs => T: 10471, W: 5970, %: 57%"
            << endl;
    EV
            << "As 10 + 2(flow10) + 2(flow 11) + 9(flow0-8) AGVs => T: 9538, W: 4830, %: 51%"
            << endl;
    EV
...
void HospitalControlApp::readLane(AGV *cur, std::string str) {
    double localWaitTime = cur->itinerary->localWaitTime * 0.1;
    int c = 0;
    std::string v;
    ...
    cur->itinerary->v = graph->searchVertex(v);
    if (cur->itinerary->prevV != cur->itinerary->v) {
...
double HospitalControlApp::getAvailablePerdestrian(std::string crossId,
        double _time) {
    int count = 0;
    double start = 0;
    ...
    auto it = find_if(crossings.begin(), crossings.end(),
            [&crossId](const Crossing &obj) {
                return obj.id.compare(crossId) == 0;
            });
    if (it != crossings.end()) {
//        EV << (it->rec).A <<endl;
        double pivot = start;
        do {
            for (auto elem : it->peoples) {
                if (pivot <= std::get < 3 > (elem)
                        && std::get < 3 > (elem) < pivot + 0.1) {
...
double HospitalControlApp::getVeloOfPerdestrian(std::string crossId,
        double _time) {
    double start = 0;
    if (_time - Constant::DELTA_T > 0) {
        start = _time - Constant::DELTA_T;
...
//EV << "Start: "<<start << " End: "<< _time <<endl;
    ...
    if (it != crossings.end()) {
        for (auto elem : it->peoples) {
            if (start <= std::get < 3 > (elem)
                    && std::get < 3 > (elem) < _time) {
        ...
        for (auto person : personIds) {
            ...
            for (auto elem : it->peoples) {
                if (std::get < 0 > (elem).compare(person) == 0
                        && start <= std::get < 3 > (elem)
                        && std::get < 3 > (elem) <= _time) {
                    tmp.push_back(elem);
                }
            }
